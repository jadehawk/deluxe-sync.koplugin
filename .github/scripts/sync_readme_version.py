...
META_PATH = ROOT / "deluxe-sync.koplugin" / "_meta.lua"
README_PATH = ROOT / "README.md"

META_VERSION_RE = re.compile(r'^\s*version\s*=\s*"([0-9]+\.[0-9]+\.[0-9]+(?:\.[0-9]+)?)"\s*,?\s*$', re.MULTILINE)
...
def main() -> None:
    ...
    if not match:
        raise SystemExit(f"Could not read supported version from {META_PATH}")
