...
local _ = require("I18N").translate

local Updater = {}

local REPO = "jadehawk/deluxe-sync.koplugin"
local LATEST_URL = "https://api.github.com/repos/" .. REPO .. "/releases/latest"
local ASSET_NAME = "deluxe-sync.koplugin.zip"
local PLUGIN_DIR_NAME = "deluxe-sync.koplugin"
...
local function request(url, sink)
    local ok, code, headers, status = https.request{
        ...
        headers = {
            ...
            ["User-Agent"] = "Deluxe-Sync-KOReader",
...
local function prepareExtractedPlugin(raw_dir, staging)
    ...
    return nil, "Update archive does not contain a valid Deluxe-Sync plugin"
end
...
local function apply(plugin_dir, release)
    ...
    local zip_path = parent .. "/deluxe-sync-update.zip"
...
local function installRelease(plugin, release)
    NetworkAction.run(plugin, NetworkMgr, UIManager, function()
        local updating_message = InfoMessage:new{ text = _("Downloading and installing Deluxe-Sync update...") }
        ...
        UIManager:show(ConfirmBox:new{
            text = _("Deluxe-Sync v") .. release.version .. _(" installed. KOReader must restart to use the update."),
...
local function promptRelease(plugin, release, automatic)
    local box = {
        text = _("Deluxe-Sync v") .. release.version .. _(" is available.") .. string.char(10, 10)
    ...
    if automatic then
        ...
        box.cancel_callback = function()
            local ok, err = plugin.store:setSkippedUpdateVersion(release.version)
            if not ok then
            ...
            UIManager:show(InfoMessage:new{
                text = _("Deluxe-Sync v") .. release.version .. _(" will be skipped.") .. string.char(10, 10)
                    .. _("You will not be prompted for this version again. You can still install it at any time from Deluxe-Sync > Check for Updates."),
                timeout = 7,
            })
        end
    end
...
function Updater.checkAutomatic(plugin)
    NetworkAction.run(plugin, NetworkMgr, UIManager, function()
        ...
        local skipped = plugin.store:getSkippedUpdateVersion()
...
function Updater.check(plugin, interactive)
    NetworkAction.run(plugin, NetworkMgr, UIManager, function()
        local checking_message = InfoMessage:new{ text = _("Checking for Deluxe-Sync updates...") }
        ...
        if not Updater.isNewer(release.version, plugin.PLUGIN_VERSION) then
            UIManager:show(InfoMessage:new{ text = _("Deluxe-Sync is up to date (v") .. plugin.PLUGIN_VERSION .. ").", timeout = 4 })
            return
        end
